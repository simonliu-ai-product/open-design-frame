...
An MCP server for [open-frame](https://github.com/simonliu-ai-product/open-frame). It lets an
...
open-frame dev --mcp        # canvas at :5274, MCP at :5274/mcp
```
...
| `edit_element` | one element's style or text, addressed by the `data-of-loc` the page reports |
...
Every tool is a thin wrapper over `@open-frame/core/ops` — the same functions the
dev server calls for the browser, including what they refuse.

## Mounting it yourself

```ts
import { createOpenFrameMcpMiddleware } from '@open-frame/mcp';

app.use('/mcp', createOpenFrameMcpMiddleware({ userCwd: process.cwd() }));
```
